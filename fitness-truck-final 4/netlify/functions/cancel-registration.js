...
async function safelyMarkRegistrationCancelled(registrationId) {
  try {
    ...
    console.error('Could not mark registration cancelled:', error);
    return false;
  }
}

async function safelyDeleteRegistration(registrationId) {
  try {
    await supabaseRequest(`/registrations?id=eq.${encodeURIComponent(registrationId)}`, {
      method: 'DELETE',
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const paymentIntentStatus = String(paymentIntent?.status || '').trim().toLowerCase();
    ...
    if (paymentIntent && String(paymentIntent.payrexx_transaction_id || '').trim()) {
      ...
      if (transactionStatus === 'refunded' || transactionStatus === 'partially-refunded' || paymentIntentStatus === 'refunded') {
        ...
        try {
          ...
          refundTriggered = true;
          refundTransactionId = String(refundTx.id || refundTx.uuid || originalTransactionId || '');
        } catch (refundError) {
    ...
    } else if (amountChf > 0 && paymentIntentStatus !== 'refunded') {
      return jsonResponse(409, {
        success: false,
    ...
    if (paymentIntent) {
      await supabaseRequest(`/payment_intents?id=eq.${encodeURIComponent(String(paymentIntent.id || ''))}`, {
        method: 'PATCH',
        body: {
          status: refundTriggered || paymentIntentStatus === 'refunded' ? 'refunded' : 'cancelled',
          registration_id: null
        }
      });
    }

    // Keep the attendance status update as a first pass for compatibility with any admin views,
    ...
    return jsonResponse(200, {
      success: true,
      refunded: refundTriggered || paymentIntentStatus === 'refunded',
      deleted,
      message: refundTriggered || paymentIntentStatus === 'refunded'
